...
class Additif:
    def __init__(self, data_preparation_object):
        ...
        mean_train_absolute_error = numpy.mean(
            numpy.abs(y_train_predicted - data_preparation_object.y_train)
        )
        ...
        mean_test_absolute_error = numpy.mean(
            numpy.abs(y_test_predicted - data_preparation_object.y_test)
        )
    ...
    def show_model_predictions(self, y_train_predicted, y_test_predicted):
        ...
        plt.plot(
            self.data_preparation_object.x_train,
            self.data_preparation_object.y_train,
            "bo:",
        )  # vt
        plt.plot(
            self.data_preparation_object.x_train, y_train_predicted, "b"
        )  # prediction

        plt.plot(
            self.data_preparation_object.x_test,
            self.data_preparation_object.y_test,
            "ro:",
        )  # vt
        plt.plot(
            self.data_preparation_object.x_test, y_test_predicted, "r"
        )  # prediction
        plt.show()
